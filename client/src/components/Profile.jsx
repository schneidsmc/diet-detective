import React from "react";

const About = () => {
  return (
    <div className="text-white max-w-[1200px] mx-auto my-12" id="about">
      <div className="sm:py-16">
        <div className="mt-4 md:mt-0 text-center flex">
          <div className="my-auto mx-6">
            <h2 className="text-4xl font-bold mb-16 dark:text-gray-600">
              Welcome, User
            </h2>
          </div>
        </div>
        <div className="text-center">
          <h1 className="text-center mb-10 font-bold text-4xl dark:text-gray-600">
            Daily intake
          </h1>
          <div
            id="day1"
            class="backdrop-filter dark:bg-gray-500 p-4 mx-auto border-solid border-2 border-gray-500 rounded-md mb-10"
          >
            <h3 id="card-name" class="text-3xl mb-10 text-center">
              Total Chart
            </h3>
            <h3 id="card-date" class="text-3xl mb-10 text-center ">
              Sunday, 5 May, 2024
            </h3>
            <p id="card-calories" className="text-2xl ">
              Total Calories: 520
            </p>
            <p id="card-sugar" className="text-2xl ">
              Total Cugar: 6 g
            </p>
            <p id="card-sodium" className="text-2xl ">
              Total Sodium: 820 mg
            </p>
            <p id="card-protein" className="text-2xl ">
              Total Protein: 25 g
            </p>
          </div>
        </div>
        <hr className="my-10" />
        <h1 className="text-center my-10 font-bold text-4xl dark:text-gray-600 ">
          Nutricion Tracker
        </h1>
        <div className="mt-4 md:mt-0 grid grid-cols-3 ">
          <div
            id="day1"
            class="backdrop-filter p-4 w-80 border-solid border-2 dark:bg-gray-500 border-gray-500 rounded-md mb-10"
          >
            <h3 id="card-name" class="text-3xl mb-10 text-center ">
              Cheeseburger
            </h3>
            <h3 id="card-date" class="text-3xl mb-10 text-center ">
              Sunday, 5 May, 2024
            </h3>
            <p id="card-calories" className="">
              Calories: 520
            </p>
            <p id="card-sugar" className="">
              Sugar: 6 g
            </p>
            <p id="card-sodium" className="">
              Sodium: 820 mg
            </p>
            <p id="card-protein" className="">
              Protein: 25 g
            </p>
          </div>
          <div
            id="day1"
            class="backdrop-filter p-4 w-80 border-solid border-2 dark:bg-gray-500 border-gray-500 rounded-md mb-10"
          >
            <h3 id="card-name" class="text-3xl mb-10 text-center ">
              Cheeseburger
            </h3>
            <h3 id="card-date" class="text-3xl mb-10 text-center ">
              Sunday, 5 May, 2024
            </h3>
            <p id="card-calories" className="">
              Calories: 520
            </p>
            <p id="card-sugar" className="">
              Sugar: 6 g
            </p>
            <p id="card-sodium" className="">
              Sodium: 820 mg
            </p>
            <p id="card-protein" className="">
              Protein: 25 g
            </p>
          </div>
          <div
            id="day1"
            class="backdrop-filter p-4 w-80 border-solid border-2 dark:bg-gray-500 border-gray-500 rounded-md mb-10"
          >
            <h3 id="card-name" class="text-3xl mb-10 text-center ">
              Cheeseburger
            </h3>
            <h3 id="card-date" class="text-3xl mb-10 text-center ">
              Sunday, 5 May, 2024
            </h3>
            <p id="card-calories" className="">
              Calories: 520
            </p>
            <p id="card-sugar" className="">
              Sugar: 6 g
            </p>
            <p id="card-sodium" className="">
              Sodium: 820 mg
            </p>
            <p id="card-protein" className="">
              Protein: 25 g
            </p>
          </div>
          <div
            id="day1"
            class="backdrop-filter p-4 w-80 border-solid border-2 dark:bg-gray-500 border-gray-500 rounded-md mb-10"
          >
            <h3 id="card-name" class="text-3xl mb-10 text-center ">
              Cheeseburger
            </h3>
            <h3 id="card-date" class="text-3xl mb-10 text-center ">
              Sunday, 5 May, 2024
            </h3>
            <p id="card-calories" className="">
              Calories: 520
            </p>
            <p id="card-sugar" className="">
              Sugar: 6 g
            </p>
            <p id="card-sodium" className="">
              Sodium: 820 mg
            </p>
            <p id="card-protein" className="">
              Protein: 25 g
            </p>
          </div>
          <div
            id="day1"
            class="backdrop-filter p-4 w-80 border-solid border-2 dark:bg-gray-500 border-gray-500 rounded-md mb-10"
          >
            <h3 id="card-name" class="text-3xl mb-10 text-center ">
              Cheeseburger
            </h3>
            <h3 id="card-date" class="text-3xl mb-10 text-center ">
              Sunday, 5 May, 2024
            </h3>
            <p id="card-calories" className="">
              Calories: 520
            </p>
            <p id="card-sugar" className="">
              Sugar: 6 g
            </p>
            <p id="card-sodium" className="">
              Sodium: 820 mg
            </p>
            <p id="card-protein" className="">
              Protein: 25 g
            </p>
          </div>
          <div
            id="day1"
            class="backdrop-filter p-4 w-80 border-solid border-2 dark:bg-gray-500 border-gray-500 rounded-md mb-10"
          >
            <h3 id="card-name" class="text-3xl mb-10 text-center ">
              Cheeseburger
            </h3>
            <h3 id="card-date" class="text-3xl mb-10 text-center ">
              Sunday, 5 May, 2024
            </h3>
            <p id="card-calories" className="">
              Calories: 520
            </p>
            <p id="card-sugar" className="">
              Sugar: 6 g
            </p>
            <p id="card-sodium" className="">
              Sodium: 820 mg
            </p>
            <p id="card-protein" className="">
              Protein: 25 g
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default About;
